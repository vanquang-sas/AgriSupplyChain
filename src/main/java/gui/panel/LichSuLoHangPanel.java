...
public class LichSuLoHangPanel extends JPanel {
    ...
    public LichSuLoHangPanel() {
        ...
        left.add(btnView); left.add(btnRefresh);
        ...
        DefaultTableCellRenderer hdrRdr = new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(JTable t, Object v, boolean isS, boolean hasF, int r, int c) {
                ...
                comp.setFont(new Font("Segoe UI", Font.BOLD, 14)); comp.setBackground(AppColor.SECONDARY_HOVER);
        ...
        for (int i = 0; i < columns.length; i++) table.getColumnModel().getColumn(i).setCellRenderer(zebraRdr);
        // Status column
        ...
        btnView.addActionListener(e -> {
            ...
            if (r < 0) { JOptionPane.showMessageDialog(this, "Vui lòng chọn một lô hàng để xem.", "Thông báo", JOptionPane.INFORMATION_MESSAGE); return; }
            String maLH = tableModel.getValueAt(table.convertRowIndexToModel(r), 0).toString();
            showDetailDialog(maLH);
        });

        table.addMouseListener(new MouseAdapter() { @Override public void mouseClicked(MouseEvent e) { if (e.getClickCount() == 2) { int r = table.rowAtPoint(e.getPoint()); if (r >= 0) { String maLH = tableModel.getValueAt(table.convertRowIndexToModel(r), 0).toString(); showDetailDialog(maLH); } } } });

        search.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            private void onSearch() { String kw = search.getText().trim(); loadHistory(kw.isEmpty() ? null : kw); }
    ...
    private JPanel buildStats() {
        ...
        lblTong = new JLabel("0"); lblChoNhap = new JLabel("0"); lblDaNhap = new JLabel("0");
    ...
    private JPanel createStatCard(String cardTitle, JLabel valueLabel, Color accent, String icon) {
        ...
        iconWrap.setOpaque(false); iconWrap.setPreferredSize(new Dimension(52, 52)); iconWrap.setLayout(new GridBagLayout());
        JLabel iconLbl = new JLabel(icon); iconLbl.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 22)); iconWrap.add(iconLbl);

        JPanel textPanel = new JPanel(new GridLayout(2, 1, 0, 2)); textPanel.setOpaque(false);
        JLabel lTitle = new JLabel(cardTitle); lTitle.setFont(new Font("Segoe UI", Font.PLAIN, 12)); lTitle.setForeground(AppColor.TEXT_SECONDARY);
    ...
    private void loadHistory() { loadHistory(null); }
    ...
    private JButton createIconButton(String svgPath) {
        ...
        try { btn.setIcon(new com.formdev.flatlaf.extras.FlatSVGIcon(svgPath, 18, 18)); } catch (Throwable ex) { btn.setText("↻"); btn.setFont(new Font("Segoe UI", Font.BOLD, 18)); }
        btn.setPreferredSize(new Dimension(36, 36)); btn.setContentAreaFilled(false); btn.setFocusPainted(false); btn.setBorderPainted(false); btn.setCursor(new Cursor(Cursor.HAND_CURSOR));
        btn.addMouseListener(new java.awt.event.MouseAdapter() { @Override public void mouseEntered(java.awt.event.MouseEvent e) { btn.putClientProperty("hover", true); btn.repaint(); } @Override public void mouseExited(java.awt.event.MouseEvent e) { btn.putClientProperty("hover", false); btn.repaint(); } });
        btn.setUI(new javax.swing.plaf.basic.BasicButtonUI() { @Override public void paint(Graphics g, JComponent c) { Graphics2D g2 = (Graphics2D) g.create(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); boolean isHovered = Boolean.TRUE.equals(c.getClientProperty("hover")); g2.setColor(isHovered ? AppColor.SECONDARY_HOVER : AppColor.SURFACE); g2.fillRoundRect(0, 0, c.getWidth(), c.getHeight(), 10, 10); g2.setColor(AppColor.BORDER); g2.setStroke(new BasicStroke(1.2f)); g2.drawRoundRect(1, 1, c.getWidth() - 3, c.getHeight() - 3, 10, 10); g2.dispose(); super.paint(g, c); } });
        return btn;
    }
    ...
    private void showDetailDialog(String maLH) {
        ...
        for (LoHangDTO lh : loHangBUS.getLichSuLoHang()) {
            if (maLH.equals(lh.getMaLH())) { current = lh; break; }
        }
        if (current == null) {
        ...
        JDialog dialog = new JDialog(SwingUtilities.getWindowAncestor(this), "Chi tiết lô hàng", Dialog.ModalityType.APPLICATION_MODAL);
        ...
        header.add(createLabel("Mã lô:")); header.add(createValueLabel(current.getMaLH()));
        header.add(createLabel("Nhà cung cấp:")); header.add(createValueLabel(current.getTenNCC() != null ? current.getTenNCC() : current.getMaNCC()));
        ...
        for (int i = 0; i < detailCols.length; i++) detailTable.getColumnModel().getColumn(i).setCellRenderer(centerRenderer);
    ...
    static class RoundedPanel extends JPanel {
        ...
        @Override protected void paintComponent(Graphics g) {
            Graphics2D g2 = (Graphics2D) g.create();
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            for (int i = 5; i >= 1; i--) { g2.setColor(new Color(0, 0, 0, 6)); g2.fill(new RoundRectangle2D.Float(i, i + 1, getWidth() - i * 2, getHeight() - i * 2, arc, arc)); }
            g2.setColor(getBackground()); g2.fill(new RoundRectangle2D.Float(0, 0, getWidth() - 5, getHeight() - 5, arc, arc)); g2.dispose(); super.paintComponent(g);
        }
    }
    ...
    static class ZebraHoverRenderer extends DefaultTableCellRenderer {
        ...
        @Override public Component getTableCellRendererComponent(JTable t, Object value, boolean selected, boolean focused, int row, int col) {
            ...
            else if (row % 2 == 0) setBackground(AppColor.SURFACE);
    ...
    static class BadgeRenderer extends DefaultTableCellRenderer {
        ...
        @Override public Component getTableCellRendererComponent(JTable t, Object value, boolean selected, boolean focused, int row, int col) {
            ...
            else if (row % 2 == 0) cell.setBackground(AppColor.SURFACE);
    ...
    static class ModernSearchField extends JTextField {
        ...
        public ModernSearchField(String placeholder) { this.placeholder = placeholder; setOpaque(false); setBorder(BorderFactory.createEmptyBorder(0, 16, 0, 36)); setFont(new Font("Segoe UI", Font.PLAIN, 13)); }
        @Override protected void paintComponent(Graphics g) { Graphics2D g2 = (Graphics2D) g.create(); g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); g2.setColor(AppColor.SURFACE); g2.fillRoundRect(0, 0, getWidth(), getHeight(), 12, 12); g2.setColor(isFocusOwner() ? AppColor.PRIMARY : AppColor.BORDER); g2.drawRoundRect(1, 1, getWidth() - 3, getHeight() - 3, 12, 12); super.paintComponent(g); if (getText().isEmpty() && !isFocusOwner()) { g2.setColor(AppColor.TEXT_SECONDARY); g2.drawString(placeholder, 16, (getHeight() - g.getFontMetrics().getHeight()) / 2 + g.getFontMetrics().getAscent()); } g2.dispose(); }
    }
